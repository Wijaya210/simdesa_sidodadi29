                    ...
                    @forelse ($pengaduans as $index => $item)
                        <tr>
                            ...
                            <td>
                                ...
                                <small
                                    class="{{ $item->is_read ? 'text-muted' : 'fw-bold text-dark' }}">{{ $item->user->nik ?? '-' }}</small>
                            </td>
                            <td>
                                <div class="{{ $item->is_read ? 'fw-bold text-dark' : 'fw-bolder text-dark' }}"
                                    style="{{ $item->is_read ? '' : 'font-weight: 800;' }}">{{ $item->judul }}</div>
                                @if($item->foto)
                                    <a href="{{ asset('storage/' . $item->foto) }}" target="_blank"
                                        class="btn btn-xs btn-outline-info mt-1" style="font-size: 0.75rem; padding: 2px 8px;">
                                        <i class="fas fa-image me-1"></i>Lihat Foto
                                    </a>
                                @endif
                            </td>
                            <td class="{{ $item->is_read ? '' : 'fw-bold' }}">{{ $item->created_at->format('d M Y') }}</td>
                            <td>
                                ...
                                <div style="max-width: 200px;">
                                    @if($item->tanggapan)
                                        <span class="text-success small"><i
                                                class="fas fa-check me-1"></i>{{ Str::limit($item->tanggapan, 50) }}</span>
                                    @else
