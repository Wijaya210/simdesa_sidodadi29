                        ...
                        <form action="{{ route('admin.berita.update', $berita->id) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal"
                                    name="tanggal" value="{{ old('tanggal', $berita->tanggal) }}" required>
                                @error('tanggal')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control @error('gambar') is-invalid @enderror" id="gambar"
                                    name="gambar" accept="image/*" onchange="previewImage()">
                                <div class="form-text">Biarkan kosong jika tidak ingin mengubah gambar. Format: JPG, JPEG,
                                    PNG. Maksimal 2MB.</div>
                                @error('gambar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                ...
                                <div class="mt-2">
                                    @if($berita->gambar)
                                        <img id="img-preview" src="{{ asset('images/berita/' . $berita->gambar) }}"
                                            alt="Preview" class="img-thumbnail" style="height: 150px;">
                                    @else
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="form-control @error('konten') is-invalid @enderror" id="konten"
                                    name="konten" rows="10" required>{{ old('konten', $berita->konten) }}</textarea>
                                @error('konten')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
